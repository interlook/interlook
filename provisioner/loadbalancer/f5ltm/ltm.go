...
const (
	AdditionalObjectDescription = "auto generated by interlook, do not edit"
)
	...
	for {
		...
		case msg := <-receive:
			...
			case comm.AddAction, comm.UpdateAction:
				updatedMsg := f5.updateService(msg)
			...
			case comm.DeleteAction:
				...
				updatedMsg := f5.deleteService(msg)
...
func (f5 *BigIP) createVirtualServer(msg comm.Message) error {
	...
	vs := bigip.VirtualServer{
		...
		Description: fmt.Sprintf("Virtual Server for %v - %v", msg.Service.Name, AdditionalObjectDescription),
	}
...
func (f5 *BigIP) updatePoolMembers(pool *bigip.Pool, msg comm.Message) error {
	...
	for _, host := range msg.Service.Hosts {
		...
		members = append(members, bigip.PoolMember{
			...
			Partition:   f5.Partition,
			Monitor:     f5.MonitorName,
			Description: fmt.Sprintf("Pool Member for %v - %v", msg.Service.Name, AdditionalObjectDescription),
		})
	}
